...
import com.opensourceautomation.android.osaextension.tasker.TaskerIntent;
...
import com.opensourceautomation.android.osaextension.utilities.TTSService;
...
public class GCMIntentService extends GCMBaseIntentService{
    ...
    protected void onMessage(Context context, Intent intent) {
        ...
        mydevicelog.log("type="+type, Log.DEBUG);
        
        if (type.equals("notification")) {
        	logMessage(context, message, category, level, osaid, messagedate);
        }
        
        if (type.equals("taskertask")){
        	executeTasker(context, message, category, level, osaid, messagedate);
        } 
    ...
    private void executeTasker(Context myContext, String task, String category, String level, String osaid, String messagedate) {
    	
    	
    	if (TaskerIntent.testStatus(this).equals(TaskerIntent.Status.OK)) {

// our gcm intent service finishes before we get a response, for now we will just execute without, if there is a problem with alot of tasks not executing we can revisit getting a response    		
...
//    					mydevicelog.log("Tasker Task ("+TaskerIntent.EXTRA_TASK_NAME+") Succeded", Log.DEBUG);
//    				else
//    					mydevicelog.log("Tasker Task ("+TaskerIntent.EXTRA_TASK_NAME+") Failed", Log.DEBUG);
//
//    				unregisterReceiver(this);
//    			}
//    		};

    		mydevicelog.log("task="+task, Log.DEBUG);
...
//    		registerReceiver( br, i.getCompletionFilter() );
    		...
    		mydevicelog.log("Tasker Status="+TaskerIntent.testStatus(this), Log.DEBUG);
    	}
    	
    	// log called tasker events to database
    	int mylevel;
		...
		initialValues.put("message", task);
		initialValues.put("category", category);
		initialValues.put("level", mylevel);
		initialValues.put("notifydate", messagedate);
		initialValues.put("notification", false);
		initialValues.put("alert", false);
		initialValues.put("speech", false);
		initialValues.put("toast", false);
		initialValues.put("popup", false);
		initialValues.put("vibrate", false);
...
//      intent.putExtra(EXTRA_DATE, messagedate);
	...
	private void generateNotification(NotificationItem mynotification, String alertsound) {
		...
		if (mynotification.getNotification()){ 
			...
			Intent resultIntent = new Intent(myContext, MessagesActivity.class);
