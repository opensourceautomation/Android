...
import com.opensourceautomation.android.osaextension.R.integer;
 ...
 */
public final class TaskerConfigureNamedScript extends AbstractPluginActivity
{
    ...
    protected void onCreate(final Bundle savedInstanceState)
    {
        ...
        setTitle(getString(R.string.plugin_namedscript));
        ...
        setContentView(R.layout.taskernamedscript);

        if (null == savedInstanceState)
        {
            if (PluginBundleManager.isNamedScriptBundleValid(localeBundle))
            {
                final String namedscript =
                        localeBundle.getString(PluginBundleManager.BUNDLE_EXTRA_STRING_NAMEDSCRIPT);
                ((EditText) findViewById(android.R.id.text1)).setText(namedscript);
            } 
        }
    }
    ...
    public void finish()
    {
        if (!isCanceled())
        {
            final String namedscript = ((EditText) findViewById(android.R.id.text1)).getText().toString();

            if (namedscript.length() > 0)
            {
                ...
                final Bundle resultBundle =
                        PluginBundleManager.generateNamedScriptBundle(getApplicationContext(), namedscript);
                resultIntent.putExtra(com.twofortyfouram.locale.Intent.EXTRA_BUNDLE, resultBundle);

                /*
                 * The blurb is concise status text to be displayed in the host's UI.
                 */
                final String blurb = generateBlurb(getApplicationContext(), namedscript);
